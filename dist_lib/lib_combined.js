...
function getLinuxAudioPci() {
  ...
  let result = [];
  try {
    const parts = execSync(cmd, execOptsLinux).toString().split('\n\n');
    parts.forEach(element => {
      const lines = element.split('\n');
      if (lines && lines.length && lines[0].toLowerCase().indexOf('audio') >= 0) {
        const audio = {};
        audio.slotId = lines[0].split(' ')[0];
        audio.driver = getValue(lines, 'Kernel driver in use', ':', true) || getValue(lines, 'Kernel modules', ':', true);
        result.push(audio);
      }
    });
    return result;
...
function audio(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        powerShell('Get-CimInstance Win32_SoundDevice | select DeviceID,StatusInfo,Name,Manufacturer | fl').then((stdout, error) => {
          if (!error) {
            const parts = stdout.toString().split(/\n\s*\n/);
            parts.forEach(element => {
              const lines = element.split('\n');
              if (getValue(lines, 'name', ':')) {
...
function parseWinBatteryPart(lines, designedCapacity, fullChargeCapacity) {
  ...
  if (status >= 0) {
    ...
    result.maxCapacity = fullChargeCapacity || parseInt(getValue(lines, 'DesignCapacity', ':') || 0);
    result.designedCapacity = parseInt(getValue(lines, 'DesignCapacity', ':') || designedCapacity);
    result.voltage = parseInt(getValue(lines, 'DesignVoltage', ':') || 0) / 1000.0;
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_darwin) {
        exec('ioreg -n AppleSmartBattery -r | egrep "CycleCount|IsCharging|DesignCapacity|MaxCapacity|CurrentCapacity|DeviceName|BatterySerialNumber|Serial|TimeRemaining|Voltage"; pmset -g batt | grep %', function (error, stdout) {
          if (stdout) {
            ...
            result.capacityUnit = result.voltage ? 'mWh' : 'mAh';
            result.maxCapacity = Math.round(parseInt('0' + getValue(lines, 'applerawmaxcapacity', '='), 10) * (result.voltage || 1));
            result.currentCapacity = Math.round(parseInt('0' + getValue(lines, 'applerawcurrentcapacity', '='), 10) * (result.voltage || 1));
            result.designedCapacity = Math.round(parseInt('0' + getValue(lines, 'DesignCapacity', '='), 10) * (result.voltage || 1));
            result.manufacturer = 'Apple';
            result.serial = getValue(lines, 'BatterySerialNumber', '=') || getValue(lines, 'Serial', '=');
            result.model = getValue(lines, 'DeviceName', '=');
            let percent = null;
      ...
      if (_windows) {
        try {
          const workload = [];
          workload.push(powerShell('Get-CimInstance Win32_Battery | select BatteryStatus, DesignCapacity, DesignVoltage, EstimatedChargeRemaining, DeviceID | fl'));
          workload.push(powerShell('(Get-WmiObject -Class BatteryStaticData -Namespace ROOT/WMI).DesignedCapacity'));
          workload.push(powerShell('(Get-CimInstance -Class BatteryFullChargedCapacity -Namespace ROOT/WMI).FullChargedCapacity'));
          promiseAll(
            workload
          ).then((data) => {
            if (data) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        try {
          const hdicon = execSync('hcitool con', execOptsLinux).toString().toLowerCase();
      ...
      if (_windows) {
        powerShell('Get-CimInstance Win32_PNPEntity | select PNPClass, Name, Manufacturer | fl').then((stdout, error) => {
          if (!error) {
            const parts = stdout.toString().split(/\n\s*\n/);
            parts.forEach((part) => {
              if (getValue(part.split('\n'), 'PNPClass', ':') === 'Bluetooth') {
      ...
      cpuFlags().then(flags => {
        ...
        if (_darwin) {
          exec('sysctl machdep.cpu hw.cpufrequency_max hw.cpufrequency_min hw.packages hw.physicalcpu_max hw.ncpu hw.tbfrequency hw.cpufamily hw.cpusubfamily', function (error, stdout) {
            ...
            let tbFrequency = getValue(lines, 'hw.tbfrequency') / 1000000000.0;
            tbFrequency = tbFrequency < 0.1 ? tbFrequency * 100 : tbFrequency;
            result.speed = result.speed === 0 ? tbFrequency : result.speed;

            _cpu_speed = result.speed;
            result = cpuBrandManufacturer(result);
            result.speedMin = getValue(lines, 'hw.cpufrequency_min') ? (getValue(lines, 'hw.cpufrequency_min') / 1000000000.0) : result.speed;
            result.speedMax = getValue(lines, 'hw.cpufrequency_max') ? (getValue(lines, 'hw.cpufrequency_max') / 1000000000.0) : result.speed;
            ...
            if (countCores && countThreads) {
              result.cores = parseInt(countThreads) || cores();
              result.physicalCores = parseInt(countCores) || cores();
            }
            cpuCache().then((res) => {
              result.cache = res;
              resolve(result);
        ...
        if (_linux) {
          ...
          exec('export LC_ALL=C; lscpu; echo -n "Governor: "; cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor 2>/dev/null; echo; unset LC_ALL', function (error, stdout) {
            ...
            result = cpuBrandManufacturer(result);
            result.vendor = cpuManufacturer(getValue(lines, 'vendor id'));
            ...
            const coresPerSocket = parseInt(getValue(lines, 'core(s) per socket'), 10); // number of cores (e.g. 16 on i12900)
            ...
            // Test Raspberry
            if (result.vendor === 'ARM' && isRaspberry()) {
            ...
            exec('export LC_ALL=C; dmidecode –t 4 2>/dev/null | grep "Upgrade: Socket"; unset LC_ALL', function (error2, stdout2) {
              ...
              if (lines2 && lines2.length) {
                result.socket = getValue(lines2, 'Upgrade').replace('Socket', '').trim() || result.socket;
              }
              resolve(result);
            });
          });
        ...
        if (_freebsd || _openbsd || _netbsd) {
          ...
          exec('export LC_ALL=C; dmidecode -t 4; dmidecode -t 7 unset LC_ALL', function (error, stdout) {
            ...
            result = cpuBrandManufacturer(result);
            result.vendor = cpuManufacturer(getValue(lines, 'manufacturer'));
            let sig = getValue(lines, 'signature');
            sig = sig.split(',');
            for (let i = 0; i < sig.length; i++) {
              sig[i] = sig[i].trim();
            }
            result.family = getValue(sig, 'Family', ' ', true);
            result.model = getValue(sig, 'Model', ' ', true);
            result.stepping = getValue(sig, 'Stepping', ' ', true);
            result.revision = '';
            const voltage = parseFloat(getValue(lines, 'voltage'));
            result.voltage = isNaN(voltage) ? '' : voltage.toFixed(2);
            for (let i = 0; i < cache.length; i++) {
              lines = cache[i].split('\n');
              let cacheType = getValue(lines, 'Socket Designation').toLowerCase().replace(' ', '-').split('-');
        ...
        if (_windows) {
          try {
            const workload = [];
            workload.push(powerShell('Get-CimInstance Win32_processor | select Name, Revision, L2CacheSize, L3CacheSize, Manufacturer, MaxClockSpeed, Description, UpgradeMethod, Caption, NumberOfLogicalProcessors, NumberOfCores | fl'));
            workload.push(powerShell('Get-CimInstance Win32_CacheMemory | select CacheType,InstalledSize,Level | fl'));
...
function getCpuCurrentSpeedSync() {
  ...
  if (cpus && cpus.length && cpus[0].hasOwnProperty('speed')) {
    ...
    try {
      const speedStrings = execSync('cat /proc/cpuinfo | grep "cpu MHz" | cut -d " " -f 3', execOptsLinux).toString().split('\n').filter((line) => line.length > 0);
...
function cpuTemperature(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          powerShell('Get-CimInstance MSAcpi_ThermalZoneTemperature -Namespace "root/wmi" | Select CurrentTemperature').then((stdout, error) => {
            if (!error) {
              let sum = 0;
...
function cpuFlags(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          exec('reg query "HKEY_LOCAL_MACHINE\\HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0" /v FeatureSet', execOptsWin, function (error, stdout) {
            if (!error) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          const workload = [];
          workload.push(powerShell('Get-CimInstance Win32_processor | select L2CacheSize, L3CacheSize | fl'));
          workload.push(powerShell('Get-CimInstance Win32_CacheMemory | select CacheType,InstalledSize,Level | fl'));
...
function parseWinCache(linesProc, linesCache) {
  ...
  parts.forEach(function (part) {
    ...
    const level = getValue(lines, 'Level');
...
function getLoad() {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      let loads = os.loadavg().map(function (x) { return x / cores(); });
      ...
      if (now >= 200) {
        ...
        if (_linux) {
          try {
            const lines = execSync('cat /proc/stat 2>/dev/null | grep cpu', execOptsLinux).toString().split('\n');
            if (lines.length > 1) {
              lines.shift();
    ...
    process.nextTick(() => {
      ...
      if (isFunction(containerIDs) && !callback) {
        ...
        try {
          ...
          containerIDsSanitized.__proto__.toString = stringToString;
          containerIDsSanitized.__proto__.substr = stringSubstr;
          containerIDsSanitized.__proto__.substring = stringSubstring;
          containerIDsSanitized.__proto__.trim = stringTrim;
          containerIDsSanitized.__proto__.startsWith = stringStartWith;
        } catch (e) {
          Object.setPrototypeOf(containerIDsSanitized, stringObj);
        }
        ...
        if (containerIDsSanitized !== '*') {
          ...
          const s = (isPrototypePolluted() ? '' : sanitizeShellString(containerIDs, true)).trim();
          const l = mathMin(s.length, 2000);
          for (let i = 0; i <= l; i++) {
            if (s[i] !== undefined) {
...
const execPromiseSave = promisifySave(require('child_process').exec);
    ...
    process.nextTick(() => {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
        ...
        if (_linux) {
          try {
            ...
            execSync('cat /proc/mounts 2>/dev/null', execOptsLinux).toString().split('\n').filter(line => {
              return line.startsWith('/');
            }).forEach((line) => {
      ...
      if (_windows) {
        try {
          ...
          powerShell(cmd).then((stdout, error) => {
            if (!error) {
      ...
      if (_freebsd || _openbsd || _netbsd || _darwin) {
        ...
        exec(cmd, { maxBuffer: 1024 * 1024 }, function (error, stdout) {
          if (!error) {
            ...
            result.allocated = parseInt(getValue(lines, 'kern.num_files', ':'), 10) || parseInt(getValue(lines, 'kern.open_files', ':'), 10);
  ...
  try {
    data.forEach(element => {
      if (element.type.startsWith('raid')) {
        const lines = execSync(`mdadm --export --detail /dev/${element.name}`, execOptsLinux).toString().split('\n');
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        ...
        try {
          // wmic('logicaldisk get Caption,Description,DeviceID,DriveType,FileSystem,FreeSpace,Name,Size,VolumeName,VolumeSerialNumber /value').then((stdout, error) => {
          // powerShell('Get-CimInstance Win32_logicaldisk | select Caption,DriveType,Name,FileSystem,Size,VolumeSerialNumber,VolumeName | fl').then((stdout, error) => {
          const workload = [];
          workload.push(powerShell('Get-CimInstance -ClassName Win32_LogicalDisk | select Caption,DriveType,Name,FileSystem,Size,VolumeSerialNumber,VolumeName | fl'));
          workload.push(powerShell('Get-WmiObject -Class Win32_diskdrive | Select-Object -Property PNPDeviceId,DeviceID, Model, Size, @{L=\'Partitions\'; E={$_.GetRelated(\'Win32_DiskPartition\').GetRelated(\'Win32_LogicalDisk\') | Select-Object -Property DeviceID, VolumeName, Size, FreeSpace}} | fl'));
          promiseAll(
            workload
          ).then((res) => {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
          ...
          if (!error) {
            try {
              ...
              try {
                ...
                try {
                  const out2 = execSync('export LC_ALL=C; lsblk -bPo NAME,TYPE,SIZE,FSTYPE,MOUNTPOINT,UUID,ROTA,RO,RM,LABEL,MODEL,OWNER,GROUP 2>/dev/null; unset LC_ALL', execOptsLinux).toString();
              ...
              devices.forEach((device) => {
                ...
                try {
                  mediumType = execSync('cat /sys/block/' + logical + '/queue/rotational 2>/dev/null', execOptsLinux).toString().split('\n')[0];
                } catch (e) {
                  noop();
                }
          ...
          if (!error) {
            ...
            try {
              ...
              devices.forEach(function (device) {
                ...
                if (sizeStr) {
                  ...
                  if (sizeValue) {
                    const smartStatusString = getValue(lines, 'S.M.A.R.T. status', ':', true).trim().toLowerCase();
                    result.push({
                      device: BSDName,
                      type: mediumType.startsWith('Solid') ? 'SSD' : 'HD',
                      name: getValue(lines, 'Model', ':', true).trim(),
                      vendor: getVendorFromModel(getValue(lines, 'Model', ':', true).trim()) || getValue(lines, 'Manufacturer', ':', true),
                      size: sizeValue,
                      bytesPerSector: null,
                      totalCylinders: null,
                      totalHeads: null,
            ...
            try {
              ...
              devices.forEach(function (device) {
                ...
                let lines = device.split('\n');
                const linkWidth = getValue(lines, 'link width', ':', true).trim();
                const sizeStr = getValue(lines, '!capacity', ':', true).trim();
                const BSDName = getValue(lines, 'BSD Name', ':', true).trim();
                if (sizeStr) {
                  let sizeValue = 0;
                  ...
                  if (sizeValue) {
                    ...
                    result.push({
                      ...
                      type: 'NVMe',
                      name: getValue(lines, 'Model', ':', true).trim(),
                      vendor: getVendorFromModel(getValue(lines, 'Model', ':', true).trim()),
                      size: sizeValue,
                      bytesPerSector: null,
                      totalCylinders: null,
                      totalHeads: null,
                ...
                if (sizeStr) {
                  ...
                  if (sizeValue) {
                    ...
                    result.push({
                      device: BSDName,
                      type: 'USB',
                      name: getValue(lines, 'Model', ':', true).trim().replaceAll(':', ''),
                      vendor: getVendorFromModel(getValue(lines, 'Model', ':', true).trim()),
                      size: sizeValue,
                      bytesPerSector: null,
                      totalCylinders: null,
      ...
      if (_windows) {
        try {
          const workload = [];
          workload.push(powerShell('Get-CimInstance Win32_DiskDrive | select Caption,Size,Status,PNPDeviceId,DeviceId,BytesPerSector,TotalCylinders,TotalHeads,TotalSectors,TotalTracks,TracksPerCylinder,SectorsPerTrack,FirmwareRevision,SerialNumber,InterfaceType | fl'));
          workload.push(powerShell('Get-PhysicalDisk | select BusType,MediaType,FriendlyName,Model,SerialNumber,Size | fl'));
          if (smartMonToolsInstalled()) {
            try {
              const smartDev = JSON.parse(execSync('smartctl --scan -j').toString());
              if (smartDev && smartDev.devices && smartDev.devices.length > 0) {
                smartDev.devices.forEach((dev) => {
                  workload.push(execPromiseSave(`smartctl -j -a ${dev.name}`, execOptsWin));
                });
              }
            } catch (e) {
              noop();
          ...
          promiseAll(
            ...
            devices.forEach(function (device) {
              ...
              const status = getValue(lines, 'Status', ':').trim().toLowerCase();
              if (size) {
                result.push({
                  device: getValue(lines, 'DeviceId', ':'),  // changed from PNPDeviceId to DeviceID (be be able to match devices)
                  type: device.indexOf('SSD') > -1 ? 'SSD' : 'HD',  // just a starting point ... better: MSFT_PhysicalDisk - Media Type ... see below
                  name: getValue(lines, 'Caption', ':'),
                  vendor: getVendorFromModel(getValue(lines, 'Caption', ':', true).trim()),
                  size: parseInt(size),
            ...
            devices.forEach(function (device) {
              ...
              const name = getValue(lines, 'FriendlyName', ':').trim().replace('Msft ', 'Microsoft');
              const size = getValue(lines, 'Size', ':').trim();
...
function graphics(callback) {
  ...
  function parseLinesLinuxControllers(lines) {
    ...
    let pciIDs = [];
    try {
      pciIDs = execSync('export LC_ALL=C; dmidecode -t 9 2>/dev/null; unset LC_ALL | grep "Bus Address: "', execOptsLinux).toString().split('\n');
  ...
  function nvidiaSmi(options) {
    ...
    if (nvidiaSmiExe) {
      ...
      try {
        const sanitized = sanitizeShellString(cmd);
    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        ...
        try {
          ...
          workload.push(powerShell('gp "HKLM:\\SYSTEM\\ControlSet001\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}\\*" -ErrorAction SilentlyContinue | where MatchingDeviceId $null -NE | select MatchingDeviceId,HardwareInformation.qwMemorySize | fl'));
          workload.push(powerShell('Get-CimInstance win32_desktopmonitor | fl *'));
          workload.push(powerShell('Get-CimInstance -Namespace root\\wmi -ClassName WmiMonitorBasicDisplayParams | fl'));
          workload.push(powerShell('Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Screen]::AllScreens'));
          workload.push(powerShell('Get-CimInstance -Namespace root\\wmi -ClassName WmiMonitorConnectionParams | fl'));
          workload.push(powerShell('gwmi WmiMonitorID -Namespace root\\wmi | ForEach-Object {(($_.ManufacturerName -notmatch 0 | foreach {[char]$_}) -join "") + "|" + (($_.ProductCodeID -notmatch 0 | foreach {[char]$_}) -join "") + "|" + (($_.UserFriendlyName -notmatch 0 | foreach {[char]$_}) -join "") + "|" + (($_.SerialNumberID -notmatch 0 | foreach {[char]$_}) -join "") + "|" + $_.InstanceName}'));
    ...
    for (const i in vections) {
      if ({}.hasOwnProperty.call(vections, i)) {
        if (vections[i].trim() !== '') {
          ...
          if (matchingDeviceId) {
            const quadWordmemorySize = parseInt(getValue(lines, 'HardwareInformation.qwMemorySize'));
    ...
    for (let i in sections) {
      if ({}.hasOwnProperty.call(sections, i)) {
        if (sections[i].trim() !== '') {
          ...
          let pnpDeviceId = getValue(lines, 'PNPDeviceID', ':').match(/PCI\\(VEN_[0-9A-F]{4})&(DEV_[0-9A-F]{4})(?:&(SUBSYS_[0-9A-F]{8}))?(?:&(REV_[0-9A-F]{2}))?/i);
  ...
  function parseLinesWindowsDisplaysPowershell(ssections, msections, dsections, tsections, isections) {
    ...
    for (let i = 0; i < ssections.length; i++) {
      if (ssections[i].trim() !== '') {
        ...
        const instanceName = getValue(linesMonitor, 'InstanceName').toLowerCase();
        const videoOutputTechnology = getValue(linesConnection, 'VideoOutputTechnology');
      ...
      for (let i = 0; i <= l; i++) {
        if (s[i] !== undefined) {
          try {
            ...
            Object.setPrototypeOf(s[i], stringObj);
          }
      ...
      try {
        if (urlSanitized && !isPrototypePolluted()) {
          try {
            ...
            Object.setPrototypeOf(urlSanitized, stringObj);
          }
          ...
          checkWebsite(urlSanitized).then((res) => {
...
function inetLatency(host, callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      let hostSanitized = '';
      const s = (isPrototypePolluted() ? '8.8.8.8' : sanitizeShellString(host, true)).trim();
      const l = mathMin(s.length, 2000);
      for (let i = 0; i <= l; i++) {
        if (!(s[i] === undefined)) {
          try {
      ...
      try {
        hostSanitized.__proto__.startsWith = stringStartWith;
      } catch (e) {
        Object.setPrototypeOf(hostSanitized, stringObj);
      }
      ...
      if (_windows) {
        ...
        try {
          ...
          execSafe('ping', params, execOptsWin).then((stdout) => {
            if (stdout) {
              let lines = stdout.split('\r\n');
              lines.shift();
              lines.forEach(function (line) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
        ...
        try {
          fs.readFile('/proc/meminfo', function (error, stdout) {
            if (!error) {
              ...
              result.writeback = parseInt(getValue(lines, 'writeback'), 10);
              result.writeback = result.writeback ? result.writeback * 1024 : 0;
              result.dirty = parseInt(getValue(lines, 'dirty'), 10);
              result.dirty = result.dirty ? result.dirty * 1024 : 0;
              result.reclaimable = parseInt(getValue(lines, 'sreclaimable'), 10);
      ...
      if (_darwin) {
        let pageSize = 4096;
        try {
          let sysPpageSize = toInt(execSync('sysctl -n vm.pagesize').toString());
        ...
        try {
          exec('vm_stat 2>/dev/null | egrep "Pages active|Pages inactive"', function (error, stdout) {
            if (!error) {
              let lines = stdout.toString().split('\n');
              result.active = (parseInt(getValue(lines, 'Pages active'), 10) || 0) * pageSize;
      ...
      if (_windows) {
        ...
        let swapused = 0;
        try {
          powerShell('Get-CimInstance Win32_PageFileUsage | Select AllocatedBaseSize, CurrentUsage').then((stdout, error) => {
            if (!error) {
              let lines = stdout.split('\r\n').filter(line => line.trim() !== '').filter((line, idx) => idx > 0);
              lines.forEach(function (line) {
                if (line !== '') {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        exec('export LC_ALL=C; dmidecode -t memory 2>/dev/null | grep -iE "Size:|Type|Speed|Manufacturer|Form Factor|Locator|Memory Device|Serial Number|Voltage|Part Number"; unset LC_ALL', function (error, stdout) {
            ...
            devices.forEach(function (device) {
              ...
              if (parseInt(getValue(lines, 'Size'), 10) > 0) {
                ...
                result.push({
                  ...
                  manufacturer: getManufacturer(getValue(lines, 'Manufacturer:')),
          ...
          if (!result.length) {
            ...
            // Try Raspberry PI
            try {
              let stdout = execSync('cat /proc/cpuinfo 2>/dev/null', execOptsLinux);
              let lines = stdout.toString().split('\n');
              let version = getValue(lines, 'revision', ':', true).toLowerCase();

              if (isRaspberry(lines)) {
                ...
                stdout = execSync('vcgencmd get_config sdram_freq 2>/dev/null', execOptsLinux);
                lines = stdout.toString().split('\n');
                ...
                stdout = execSync('vcgencmd measure_volts sdram_p 2>/dev/null', execOptsLinux);
                lines = stdout.toString().split('\n');
      ...
      if (_darwin) {
        exec('system_profiler SPMemoryDataType', function (error, stdout) {
          ...
          if (!result.length) {
            ...
            const manufacturerId = getValue(lines, '      Manufacturer:');
            if (size && type) {
              result.push({
                size: size * 1024 * 1024 * 1024,
                bank: '0',
      ...
      if (_windows) {
        ...
        try {
          powerShell('Get-CimInstance Win32_PhysicalMemory | select DataWidth,TotalWidth,Capacity,BankLabel,MemoryType,SMBIOSMemoryType,ConfiguredClockSpeed,Speed,FormFactor,Manufacturer,PartNumber,SerialNumber,ConfiguredVoltage,MinVoltage,MaxVoltage,Tag | fl').then((stdout, error) => {
            if (!error) {
              let devices = stdout.toString().split(/\n\s*\n/);
              devices.shift();
              devices.forEach(function (device) {
                ...
                const tag = getValue(lines, 'Tag', ':');
                const tagInt = splitByNumber(tag);
                if (size) {
                  result.push({
                    size,
                    bank: getValue(lines, 'BankLabel', ':') + (tagInt[1] ? '/' + tagInt[1] : ''), // BankLabel
...
function getDefaultNetworkInterface() {
  ...
  try {
    ...
    if (_linux) {
      ...
      let result = execSync(cmd, execOptsLinux);
...
function getMacAddresses() {
  ...
  if (_linux || _freebsd || _openbsd || _netbsd) {
    if (typeof pathToIp === 'undefined') {
      try {
        const lines = execSync('which ip', execOptsLinux).toString().split('\n');
    ...
    try {
      ...
      let res = execSync(cmd, execOptsLinux);
    ...
    try {
      if ({}.hasOwnProperty.call(sections, i)) {
        ...
        if (sections[i].trim() !== '') {
          ...
          let ifacename = getValue(lines, 'Name', ':').replace(/\]/g, ')').replace(/\[/g, '(');
          let iface = getValue(lines, 'NetConnectionID', ':').replace(/\]/g, ')').replace(/\[/g, '(');
          if (ifacename.toLowerCase().indexOf('wi-fi') >= 0 || ifacename.toLowerCase().indexOf('wireless') >= 0) {
            adapterType = 'wireless';
          }
          if (netEnabled !== '') {
            const speed = parseInt(getValue(lines, 'speed', ':').trim(), 10) / 1000000;
            nics.push({
...
function getWindowsWirelessIfaceSSID(interfaceName) {
  try {
    const result = execSync(`netsh wlan show  interface name="${interfaceName}" | findstr "SSID"`, execOptsWin);
  ...
  if (connectionType == 'wired' && ifaces.length > 0) {
    ...
    try {
      ...
      if (SSID !== 'Unknown') {
        ...
        let ifaceSanitized = '';
        const s = isPrototypePolluted() ? '---' : sanitizeShellString(SSID);
        const l = mathMin(s.length, 32);
        ...
        i8021xState = execSync(`netsh wlan show profiles "${ifaceSanitized}" | findstr "802.1X"`, execOptsWin);
        i8021xProtocol = execSync(`netsh wlan show profiles "${ifaceSanitized}" | findstr "EAP"`, execOptsWin);
      }
...
function getLinuxIfaceConnectionName(interfaceName) {
  ...
  try {
    const result = execSync(cmd, execOptsLinux).toString();
...
function checkLinuxDCHPInterfaces(file) {
  ...
  try {
    ...
    const lines = execSync(cmd, execOptsLinux).toString().split('\n');
...
function getLinuxIfaceDHCPstatus(iface, connectionName, DHCPNics) {
  ...
  if (connectionName) {
    ...
    try {
      const lines = execSync(cmd, execOptsLinux).toString();
        ...
        if ((JSON.stringify(ifaces) === JSON.stringify(_ifaces)) && !rescan) {
          ...
          for (let dev in ifaces) {
            ...
            echo -n "wirelessspeed: "; iw dev ${ifaceSanitized} link 2>&1 | grep bitrate; echo;`;
              ...
              let lines = [];
              try {
                lines = execSync(cmd, execOptsLinux).toString().split('\n');
              ...
              let wirelessspeed = getValue(lines, 'wirelessspeed').split('tx bitrate: ');
        ...
        if ((JSON.stringify(ifaces) === JSON.stringify(_ifaces)) && !rescan) {
          ...
          getWindowsNics().then(function (nics) {
            ...
            for (let dev in ifaces) {
              ...
              let ifaceSanitized = '';
              const s = isPrototypePolluted() ? '---' : sanitizeShellString(dev);
              const l = mathMin(s.length, 2000);
              for (let i = 0; i <= l; i++) {
                if (s[i] !== undefined) {
                  ifaceSanitized = ifaceSanitized + s[i];
    ...
    process.nextTick(() => {
      ...
      if (isFunction(ifaces) && !callback) {
        ...
        try {
          ...
          ifaces.__proto__.toString = stringToString;
          ifaces.__proto__.substr = stringSubstr;
          ifaces.__proto__.substring = stringSubstring;
          ifaces.__proto__.trim = stringTrim;
          ifaces.__proto__.startsWith = stringStartWith;
        } catch (e) {
          Object.setPrototypeOf(ifaces, stringObj);
        }
    ...
    for (let i in sections) {
      if ({}.hasOwnProperty.call(sections, i)) {
        if (sections[i].trim() !== '') {
          let lines = sections[i].trim().split('\r\n');
          perfData.push({
            name: getValue(lines, 'Name', ':').replace(/[()[\] ]+/g, '').replace(/#|\//g, '_').toLowerCase(),
            rx_bytes: parseInt(getValue(lines, 'BytesReceivedPersec', ':'), 10),
            rx_errors: parseInt(getValue(lines, 'PacketsReceivedErrors', ':'), 10),
            rx_dropped: parseInt(getValue(lines, 'PacketsReceivedDiscarded', ':'), 10),
            tx_bytes: parseInt(getValue(lines, 'BytesSentPersec', ':'), 10),
            tx_errors: parseInt(getValue(lines, 'PacketsOutboundErrors', ':'), 10),
            tx_dropped: parseInt(getValue(lines, 'PacketsOutboundDiscarded', ':'), 10)
          });
        }
      }
    }
    ...
    process.nextTick(() => {
      ...
      if (!_network[ifaceSanitized] || (_network[ifaceSanitized] && !_network[ifaceSanitized].ms) || (_network[ifaceSanitized] && _network[ifaceSanitized].ms && Date.now() - _network[ifaceSanitized].ms >= 500)) {
        ...
        if (_windows) {
          ...
          // Performance Data
          powerShell('Get-CimInstance Win32_PerfRawData_Tcpip_NetworkInterface | select Name,BytesReceivedPersec,PacketsReceivedErrors,PacketsReceivedDiscarded,BytesSentPersec,PacketsOutboundErrors,PacketsOutboundDiscarded | fl').then((stdout, error) => {
            if (!error) {
      ...
      if (_windows) {
        try {
          exec('netstat -r', execOptsWin, function (error, stdout) {
            ...
            if (!result) {
              powerShell('Get-CimInstance -ClassName Win32_IP4RouteTable | Where-Object { $_.Destination -eq \'0.0.0.0\' -and $_.Mask -eq \'0.0.0.0\' }')
                .then((data) => {
                  ...
                  if (lines.length > 1 && !result) {
                    ...
                    resolve(result);
                    // } else {
                    //   exec('ipconfig', execOptsWin, function (error, stdout) {
                    //     let lines = stdout.toString().split('\r\n');
                    //     lines.forEach(function (line) {
...
function time() {
  ...
  if (_darwin || _linux) {
    try {
      const stdout = execSync('date +%Z && date +%z && ls -l /etc/localtime 2>/dev/null', execOptsLinux);
      const lines = stdout.toString().split(os.EOL);
...
function getFQDN() {
  ...
  if (_linux || _darwin) {
    try {
      const stdout = execSync('hostname -f 2>/dev/null', execOptsLinux);
  ...
  if (_windows) {
    try {
      const stdout = execSync('echo %COMPUTERNAME%.%USERDNSDOMAIN%', execOptsWin);
    ...
    process.nextTick(() => {
      ...
      if (_freebsd || _openbsd || _netbsd) {
        ...
        exec('sysctl kern.ostype kern.osrelease kern.osrevision kern.hostuuid machdep.bootmethod kern.geom.confxml', function (error, stdout) {
          ...
          const serial = getValue(lines, 'kern.uuid');
      ...
      if (_darwin) {
        exec('sw_vers; sysctl kern.ostype kern.osrelease kern.osrevision kern.uuid', function (error, stdout) {
          ...
          result.release = (getValue(lines, 'ProductVersion', ':', true, true) + ' ' + getValue(lines, 'ProductVersionExtra', ':', true, true)).trim();
      ...
      if (_windows) {
        ...
        try {
          const workload = [];
          workload.push(powerShell('Get-CimInstance Win32_OperatingSystem | select Caption,SerialNumber,BuildNumber,ServicePackMajorVersion,ServicePackMinorVersion | fl'));
          workload.push(powerShell('(Get-CimInstance Win32_ComputerSystem).HypervisorPresent'));
...
function isUefiWindows() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      try {
        exec('findstr /C:"Detected boot environment" "%windir%\\Panther\\setupact.log"', execOptsWin, function (error, stdout) {
          if (!error) {
            const line = stdout.toString().split('\n\r')[0];
            return resolve(line.toLowerCase().indexOf('efi') >= 0);
          } else {
            exec('echo %firmware_type%', execOptsWin, function (error, stdout) {
              if (!error) {
      ...
      try {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, 'postgresql')) {
          if (_linux) {
            ...
            if (_windows) {
              powerShell('Get-CimInstance Win32_Service | select caption | fl').then((stdout) => {
                let serviceSections = stdout.split(/\n\s*\n/);
                serviceSections.forEach((item) => {
                  if (item.trim() !== '') {
                    let lines = item.trim().split('\r\n');
                    let srvCaption = getValue(lines, 'caption', ':', true).toLowerCase();
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, 'python')) {
          if (_darwin) {
            try {
              ...
              if ((darwinXcodeExists() && semverCompare('12.0.1', osVersion) < 0) || gitHomebrewExists1 || gitHomebrewExists2) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      if (_windows) {
        try {
          const result = 'CMD';
          powerShell(`Get-CimInstance -className win32_process | where-object {$_.ProcessId -eq ${process.ppid} } | select Name`).then(stdout => {
            let result = 'CMD';
            if (stdout) {
...
echo -n "hardware: "; cat /sys/class/dmi/id/product_uuid 2> /dev/null; echo;`;
      ...
      if (_freebsd || _openbsd || _netbsd) {
        exec('sysctl -i kern.hostid kern.hostuuid', function (error, stdout) {
          const lines = stdout.toString().split('\n');
          result.hardware = getValue(lines, 'kern.hostid', ':').toLowerCase();
          result.os = getValue(lines, 'kern.hostuuid', ':').toLowerCase();
      ...
      if (_windows) {
        ...
        powerShell('Get-CimInstance Win32_ComputerSystemProduct | select UUID | fl').then((stdout) => {
          let lines = stdout.split('\r\n');
          result.hardware = getValue(lines, 'uuid', ':').toLowerCase();
          exec(`${sysdir}\\reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography" /v MachineGuid`, execOptsWin, function (error, stdout) {
...
function parseLinuxCupsPrinter(lines) {
  ...
  result.local = getValue(lines, 'Location', ' ').toLowerCase().startsWith('local');
  result.default = null;
  result.shared = getValue(lines, 'Shared', ' ').toLowerCase().startsWith('yes');
...
function parseLinuxLpstatPrinter(lines, id) {
  ...
  result.local = getValue(lines, 'Location', ':', true).toLowerCase().startsWith('local');
  result.default = null;
  result.shared = getValue(lines, 'Shared', ' ').toLowerCase().startsWith('yes');
...
function printer(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        powerShell('Get-CimInstance Win32_Printer | select PrinterStatus,Name,DriverName,Local,Default,Shared | fl').then((stdout, error) => {
          if (!error) {
            const parts = stdout.toString().split(/\n\s*\n/);
            for (let i = 0; i < parts.length; i++) {
    ...
    process.nextTick(() => {
      ...
      if (srv) {
        ...
        try {
          ...
          srvString.__proto__.toString = stringToString;
          srvString.__proto__.substr = stringSubstr;
          srvString.__proto__.substring = stringSubstring;
          srvString.__proto__.trim = stringTrim;
          srvString.__proto__.startsWith = stringStartWith;
        } catch (e) {
          Object.setPrototypeOf(srvString, stringObj);
        }
        ...
        if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
          if ((_linux || _freebsd || _openbsd || _netbsd) && srvString === '*') {
            try {
              const tmpsrv = execSync('systemctl --all --type=service --no-legend 2> /dev/null', execOptsLinux).toString().split('\n');
              srvs = [];
              ...
              try {
                srvString = '';
                const tmpsrv = execSync('service --status-all 2> /dev/null', execOptsLinux).toString().split('\n');
                for (const s of tmpsrv) {
                  const parts = s.split(']');
                  if (parts.length === 2) {
                ...
                try {
                  const srvStr = execSync('ls /etc/init.d/ -m 2> /dev/null', execOptsLinux).toString().split('\n').join('');
                  srvString = '';
                  if (srvStr) {
              ...
              if (!error) {
                ...
                serviceSections.forEach((element) => {
                  if (element.trim() !== '') {
                    ...
                    let srvName = getValue(lines, 'Name', ':', true).toLowerCase();
                    let srvCaption = getValue(lines, 'Caption', ':', true).toLowerCase();
    ...
    process.nextTick(() => {
      ...
      if ((_processes_cpu.ms && Date.now() - _processes_cpu.ms >= 500) || _processes_cpu.ms === 0) {
        if (_linux || _freebsd || _openbsd || _netbsd || _darwin || _sunos) {
          ...
          try {
            powerShell('Get-CimInstance Win32_Process | select-Object ProcessId,ParentProcessId,ExecutionState,Caption,CommandLine,ExecutablePath,UserModeTime,KernelModeTime,WorkingSetSize,Priority,PageFileUsage, @{n="CreationDate";e={$_.CreationDate.ToString("yyyy-MM-dd HH:mm:ss")}} | fl').then((stdout, error) => {
              if (!error) {
                ...
                processSections.forEach((element) => {
                  if (element.trim() !== '') {
                    let lines = element.trim().split('\r\n');
                    let pid = parseInt(getValue(lines, 'ProcessId', ':', true), 10);
                    let parentPid = parseInt(getValue(lines, 'ParentProcessId', ':', true), 10);
                    ...
                    let commandPath = getValue(lines, 'ExecutablePath', ':', true);
                    let utime = parseInt(getValue(lines, 'UserModeTime', ':', true), 10);
                    let stime = parseInt(getValue(lines, 'KernelModeTime', ':', true), 10);
                    let memw = parseInt(getValue(lines, 'WorkingSetSize', ':', true), 10);
                    ...
                    procs.push({
                      ...
                      priority: parseInt(getValue(lines, 'Priority', ':', true), 10),
                      memVsz: parseInt(getValue(lines, 'PageFileUsage', ':', true), 10),
                      memRss: Math.floor(parseInt(getValue(lines, 'WorkingSetSize', ':', true), 10) / 1024),
                      nice: 0,
                      started: getValue(lines, 'CreationDate', ':', true),
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      try {
        ...
        processesString.__proto__.toString = stringToString;
        processesString.__proto__.substr = stringSubstr;
        processesString.__proto__.substring = stringSubstring;
        processesString.__proto__.trim = stringTrim;
        processesString.__proto__.startsWith = stringStartWith;

      } catch (e) {
        Object.setPrototypeOf(processesString, stringObj);
      }
      ...
      const procSanitized = isPrototypePolluted() ? '' : (sanitizeShellString(proc) || '*');
      ...
      if (procSanitized && processes.length && processes[0] !== '------') {
        if (_windows) {
          try {
            powerShell('Get-CimInstance Win32_Process | select ProcessId,Caption,UserModeTime,KernelModeTime,WorkingSetSize | fl').then((stdout, error) => {
              if (!error) {
                let processSections = stdout.split(/\n\s*\n/);
                let procStats = [];
                let list_new = {};
                ...
                processSections.forEach((element) => {
                  if (element.trim() !== '') {
                    ...
                    let utime = parseInt(getValue(lines, 'UserModeTime', ':', true), 10);
                    let stime = parseInt(getValue(lines, 'KernelModeTime', ':', true), 10);
                    let mem = parseInt(getValue(lines, 'WorkingSetSize', ':', true), 10);
                    allcpuu = allcpuu + utime;
                    allcpus = allcpus + stime;
    ...
    process.nextTick(() => {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        exec('export LC_ALL=C; dmidecode -t system 2>/dev/null; unset LC_ALL', function (error, stdout) {
          ...
          result.model = cleanDefaults(getValue(lines, 'product name'));
          result.version = cleanDefaults(getValue(lines, 'version'));
          result.serial = cleanDefaults(getValue(lines, 'serial number'));
          result.uuid = cleanDefaults((getValue(lines, 'uuid'))).toLowerCase();
          result.sku = cleanDefaults(getValue(lines, 'sku number'));
          // Non-Root values
          ...
          try {
            lines = execSync(cmd, execOptsLinux).toString().split('\n');
            result.manufacturer = cleanDefaults(result.manufacturer === '' ? getValue(lines, 'sys_vendor') : result.manufacturer);
            result.model = cleanDefaults(result.model === '' ? getValue(lines, 'product_name') : result.model);
            result.version = cleanDefaults(result.version === '' ? getValue(lines, 'product_version') : result.version);
            result.serial = cleanDefaults(result.serial === '' ? getValue(lines, 'product_serial') : result.serial);
            result.uuid = cleanDefaults(result.uuid === '' ? getValue(lines, 'product_uuid').toLowerCase() : result.uuid);
          } catch (e) {
            noop();
          }
          ...
          if (!result.virtual) {
            try {
              const disksById = execSync('ls -1 /dev/disk/by-id/ 2>/dev/null; pciconf -lv  2>/dev/null', execOptsLinux).toString();
          ...
          if (_freebsd || _openbsd || _netbsd) {
            try {
              const lines = execSync('sysctl -i kern.hostuuid kern.hostid hw.model', execOptsLinux).toString().split('\n');
              if (!result.uuid) {
          ...
          if ((_freebsd || _openbsd || _netbsd) && !result.virtualHost) {
            try {
              const procInfo = execSync('dmidecode -t 4', execOptsLinux);
      ...
      if (_darwin) {
        exec('ioreg -c IOPlatformExpertDevice -d 2', function (error, stdout) {
          if (!error) {
            ...
            // const version = getValue(lines, 'version', '=', true);
            ...
            result.sku = getValue(lines, 'board-id', '=', true) || getValue(lines, 'target-sub-type', '=', true);
          }
          if (callback) { callback(result); }
          resolve(result);
        });
      ...
      if (_windows) {
        try {
          powerShell('Get-CimInstance Win32_ComputerSystemProduct | select Name,Vendor,Version,IdentifyingNumber,UUID | fl').then((stdout, error) => {
            if (!error) {
              let lines = stdout.split('\r\n');
              ...
              powerShell('Get-CimInstance MS_Systeminformation -Namespace "root/wmi" | select systemsku | fl ').then((stdout, error) => {
                if (!error) {
                  let lines = stdout.split('\r\n');
                  result.sku = getValue(lines, 'systemsku', ':');
                }
                if (!result.virtual) {
                  powerShell('Get-CimInstance Win32_bios | select Version, SerialNumber, SMBIOSBIOSVersion').then((stdout, error) => {
                    if (!error) {
...
function bios(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        try {
          powerShell('Get-CimInstance Win32_bios | select Description,Version,Manufacturer,@{n="ReleaseDate";e={$_.ReleaseDate.ToString("yyyy-MM-dd")}},BuildNumber,SerialNumber,SMBIOSBIOSVersion | fl').then((stdout, error) => {
            if (!error) {
              let lines = stdout.toString().split('\r\n');
    ...
    process.nextTick(() => {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        ...
        promiseAll(
          ...
          result.version = cleanDefaults(getValue(lines, 'Version'));
          result.serial = cleanDefaults(getValue(lines, 'Serial Number'));
          result.assetTag = cleanDefaults(getValue(lines, 'Asset Tag'));
          // Non-Root values
          ...
          try {
            lines = execSync(cmd, execOptsLinux).toString().split('\n');
            result.manufacturer = cleanDefaults(!result.manufacturer ? getValue(lines, 'board_vendor') : result.manufacturer);
            result.model = cleanDefaults(!result.model ? getValue(lines, 'board_name') : result.model);
            result.version = cleanDefaults(!result.version ? getValue(lines, 'board_version') : result.version);
            result.serial = cleanDefaults(!result.serial ? getValue(lines, 'board_serial') : result.serial);
            result.assetTag = cleanDefaults(!result.assetTag ? getValue(lines, 'board_asset_tag') : result.assetTag);
          } catch (e) {
            noop();
          }
      ...
      if (_windows) {
        try {
          ...
          workload.push(powerShell('Get-CimInstance Win32_baseboard | select Model,Manufacturer,Product,Version,SerialNumber,PartNumber,SKU | fl'));
          workload.push(powerShell(`Get-CimInstance Win32_physicalmemoryarray | select ${maxCapacityAttribute}, MemoryDevices | fl`));
          promiseAll(
            workload
          ).then((data) => {
            let lines = data.results[0] ? data.results[0].toString().split('\r\n') : [''];
            ...
            if (!result.model) {
              result.model = cleanDefaults(getValue(lines, 'product', ':'));
            }
            result.version = cleanDefaults(getValue(lines, 'version', ':'));
            result.serial = cleanDefaults(getValue(lines, 'serialnumber', ':'));
            result.assetTag = cleanDefaults(getValue(lines, 'partnumber', ':'));
            if (!result.assetTag) {
              result.assetTag = cleanDefaults(getValue(lines, 'sku', ':'));
            }

            // memphysical
            lines = data.results[1] ? data.results[1].toString().split('\r\n') : [''];
            result.memMax = toInt(getValue(lines, maxCapacityAttribute, ':')) * (win10plus ? 1024 : 1) || null;
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        ...
        exec(cmd, function (error, stdout) {
          let lines = stdout.toString().split('\n');
          result.manufacturer = cleanDefaults(getValue(lines, 'chassis_vendor'));
          ...
          result.version = cleanDefaults(getValue(lines, 'chassis_version'));
          result.serial = cleanDefaults(getValue(lines, 'chassis_serial'));
          result.assetTag = cleanDefaults(getValue(lines, 'chassis_asset_tag'));
      ...
      if (_darwin) {
        exec('ioreg -c IOPlatformExpertDevice -d 2', function (error, stdout) {
          if (!error) {
            ...
            result.version = model.version;
            result.serial = getValue(lines, 'ioplatformserialnumber', '=', true);
            result.assetTag = getValue(lines, 'board-id', '=', true) || getValue(lines, 'target-type', '=', true);
            result.sku = getValue(lines, 'target-sub-type', '=', true);
          }
      ...
      if (_windows) {
        try {
          powerShell('Get-CimInstance Win32_SystemEnclosure | select Model,Manufacturer,ChassisTypes,Version,SerialNumber,PartNumber,SKU,SMBIOSAssetTag | fl').then((stdout, error) => {
            if (!error) {
              let lines = stdout.toString().split('\r\n');
              ...
              result.assetTag = cleanDefaults(getValue(lines, 'partnumber', ':'));
              if (!result.assetTag) {
                result.assetTag = cleanDefaults(getValue(lines, 'SMBIOSAssetTag', ':'));
              }
...
function parseWindowsUsb(lines, id) {
  const usbType = getWindowsUsbTypeCreation(getValue(lines, 'CreationClassName', ':').toLowerCase(), getValue(lines, 'name', ':').toLowerCase());
...
function usb(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_windows) {
        powerShell('Get-CimInstance CIM_LogicalDevice | where { $_.Description -match "USB"} | select Name,CreationClassName,DeviceId,Manufacturer | fl').then((stdout, error) => {
          if (!error) {
            const parts = stdout.toString().split(/\n\s*\n/);
            for (let i = 0; i < parts.length; i++) {
...
function wifiChannelFromFrequencs(frequency) {
  ...
  for (let key in _wifi_frequencies) {
    if ({}.hasOwnProperty.call(_wifi_frequencies, key)) {
      if (_wifi_frequencies[key] === frequency) { channel = toInt(key); }
    }
  }
  return channel;
}
...
function ifaceListLinux() {
  ...
  try {
    const all = execSync(cmd, execOptsLinux).toString().split('\n').map(line => line.trim()).join('\n');
    ...
    try {
      const all = execSync('nmcli -t -f general,wifi-properties,wired-properties,interface-flags,capabilities,nsp device show 2>/dev/null', execOptsLinux).toString();
      const parts = all.split('\n\n');
      let i = 1;
      parts.forEach(ifaceDetails => {
        const lines = ifaceDetails.split('\n');
        const iface = getValue(lines, 'GENERAL.DEVICE');
        const type = getValue(lines, 'GENERAL.TYPE');
        const id = i++; // // getValue(lines, 'GENERAL.PATH');
...
function getWifiNetworkListNmi() {
  ...
  try {
    const stdout = execSync(cmd, execOptsLinux);
...
function getWifiNetworkListIw(iface) {
  const result = [];
  try {
    let iwlistParts = execSync(`export LC_ALL=C; iwlist ${iface} scan 2>&1; unset LC_ALL`, execOptsLinux).toString().split('        Cell ');
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        if (result.length === 0) {
          try {
            const iwconfigParts = execSync('export LC_ALL=C; iwconfig 2>/dev/null; unset LC_ALL', execOptsLinux).toString().split('\n\n');
            let iface = '';
    ...
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        ifaces.forEach(ifaceDetail => {
          let ifaceSanitized = '';
          const s = isPrototypePolluted() ? '---' : sanitizeShellString(ifaceDetail.iface, true);
          const ll = mathMin(s.length, 2000);
        ...
        exec(cmd, function (error, stdout) {
          try {
            const parts = stdout.toString().split('######');
            const profilerObj = plistParser(parts[0]);
        ...
        powerShell(cmd).then(function (stdout) {
          ...
          parts.forEach(part => {
            ...
            if (lines.length >= 5) {
              ...
              const channel = getValue(lines, 'Channel', ':', true) || getValue(lines, 'Canal', ':', true) || getValue(lines, 'Kanal', ':', true) || null;
...
function getAllData(srv, iface, callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (iface && isFunction(iface) && !callback) {
        callback = iface;
        iface = '';
      }

      if (srv && isFunction(srv) && !iface && !callback) {
        callback = srv;
        srv = '';
        iface = '';
      }
